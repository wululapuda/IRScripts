...
public final class ScriptButtonOverlay {
    ...
    public static void drawIfRiding(GuiScreen gui, int mouseX, int mouseY) {
        EntityRollingStock stock = getRidingStock();
        if (stock != null) {
            draw(gui, stock, mouseX, mouseY);
        }
    }

    public static boolean handleClickIfRiding(int mouseX, int mouseY, int mouseButton) {
        EntityRollingStock stock = getRidingStock();
        return stock != null && handleClick(stock, mouseX, mouseY, mouseButton);
    }

    public static boolean shouldShowStockId(UUID stockId) {
        EntityRollingStock stock = resolveStock(stockId);
        return stock != null && shouldShow(stock);
    }

    public static void drawForStockId(GuiScreen gui, UUID stockId, int mouseX, int mouseY) {
        EntityRollingStock stock = resolveStock(stockId);
        if (stock != null) {
            draw(gui, stock, mouseX, mouseY);
        }
    ...
    public static boolean handleClickForStockId(UUID stockId, int mouseX, int mouseY, int mouseButton) {
        EntityRollingStock stock = resolveStock(stockId);
        return stock != null && handleClick(stock, mouseX, mouseY, mouseButton);
    }

    private static EntityRollingStock resolveStock(UUID stockId) {
        if (stockId == null || MinecraftClient.getPlayer() == null) {
            return null;
        }
        return MinecraftClient.getPlayer().getWorld().getEntity(stockId, EntityRollingStock.class);
    }
